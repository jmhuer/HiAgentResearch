...
RESEARCH_HYPOTHESES: list[dict] = [
    {
        ...
        "hypothesis": (
            "h8 moved ResNetTrunk/MnistEncoder layer1 to ReLU but stem kwta1 remains "
            "the sole KWTA in the ensemble trunk forward (model.py ResNetTrunk lines "
            "158-175). Each KWTA(k=10) runs torch.topk (kwta.py) on 64x28x28 stem maps—"
            "the largest remaining sparse stage. baseline.json requires latency_ms <= 13.0 "
            "at accuracy >= 0.985. Replacing stem kwta1 with ReLU in ResNetTrunk and "
            "MnistEncoder removes the last trunk topk pass. Expected: latency_ms drops "
            "vs h8 with accuracy >= 0.985."
        ),
        "planned_change": (
            "In mnist/pipeline/model.py: replace ResNetTrunk and MnistEncoder stem "
            "kwta1 with nn.ReLU(inplace=True); retrain ensemble and re-measure latency_ms."
        ),
        "run_id": "run_d766c025a425",
        "timestamp": "2026-05-25T15:30:00.000000+00:00",
    },
    {
    ...
    },
    {
        ...
        "hypothesis": (
            "h6 shared ResNetTrunk plus SharedLayer4MultiLinearHead removed redundant layer4 "
            "conv passes, but ResNetTrunk still applies KWTA(k=10) in stem, layer1, and layer2 "
            "(9 torch.topk calls per forward in kwta.py; layer2 maps are 128x14x14 = 25,088 "
            "elements). baseline.json requires latency_ms <= 13.0 at accuracy >= 0.985. "
            "Switching trunk layer2 BasicBlocks to ReLU (matching h4 layer3/layer4) removes "
            "4 topk passes while keeping KWTA on the largest early feature maps. Expected: "
            "latency_ms drops vs h6 with accuracy >= 0.985."
        ),
        "planned_change": (
            "In mnist/pipeline/model.py: set ResNetTrunk and MnistEncoder layer2 "
            "_make_layer activation to relu; retrain ensemble and re-measure latency_ms."
        ),
        "run_id": "run_ea6f94bf9ad7",
