...
A passing CI run means the eval authority executed cleanly. Metric bounds and output expectations in `config.yaml` define whether the experiment improved baseline. If a valid experiment regresses or stays neutral, the next cycle should continue, pivot, repair, reset, or explicitly revert based on the intent packet rather than treating the experiment as infrastructure failure.
